...
namespace MSTest.PetzoldComputer
{
    ...
    public class Phase3ComputerTest
    {
        ...
        public void Computer()
        {
            ...
            computer.Clk.Changed += clk => { if (clk.V == VoltageSignal.LOW) Trace.TraceInformation($"PC: {computer.PC}; Output: {computer}; Bulbs: {computer.Panel.Bulbs}"); };

            // show starting state
            Trace.TraceInformation($"PC: {computer.PC}; Output: {computer}; Bulbs: {computer.Panel.Bulbs}");
            // and...go!
            computer.Oscillator.Start();  // synchronous--doesn't return until done
        }

        private static void LoadComputerRAM(Phase3Computer computer, uint nBytes)
        {
